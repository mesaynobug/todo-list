...
class JsonDatabase implements Database {
    ...
    async read(id: number): Promise<Task> {
        let readTask: Task | undefined = this.tasks.find(
            (task) => task.getId() == id
        );
    ...
    async update(id: number, task: Task): Promise<Boolean> {
        let updateIndex: number | undefined = this.tasks.findIndex(
            (task) => task.getId() == id
        );
        if (updateIndex !== undefined) {
            this.tasks[updateIndex] = task;
    ...
    async delete(id: number): Promise<Boolean> {
        let deleteTask: Task | undefined = this.tasks.find(
            (task) => task.getId() == id
        );
        if (deleteTask !== undefined) {
            this.tasks = this.tasks.filter((task) => task !== deleteTask);
...
createServer(function (req: IncomingMessage, res: ServerResponse) {
    ...
    if (req.method === "POST") {
        ...
        req.on("end", () => {
            ...
            let varCommand: string = usp
                .get("textBox")!
                .substring(0, usp.get("textBox")!.indexOf(" "));
            if (usp.get("textBox")!.indexOf(" ") == -1) {
                varCommand = usp.get("textBox")!;
            }
            let argument: string = usp
                .get("textBox")!
                .substring(varCommand.length + 1);
            let command: Command;
